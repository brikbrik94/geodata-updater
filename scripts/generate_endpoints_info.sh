...
log_section "INFO: GENERATE ENDPOINTS JSON"

# Config-Variablen (mit Fallback auf utils.sh Werte)
...
# Exportieren für Python
export TILES_DIR ASSETS_DIR INFO_OUTPUT TILES_BASE_URL ASSETS_BASE_URL
...
_assets_base_url = os.environ.get("ASSETS_BASE_URL", "https://tiles.oe5ith.at/assets").rstrip("/")
...
def collect_files(
    ...
    if root.exists():
        for path in sorted(p for p in root.rglob("*") if p.is_file()):
            ...
            # Deine Original-Logik: Prefix entfernen (z.B. osm/pmtiles -> pmtiles)
            if drop_tileset_prefix and len(rel_path.parts) > 1:
                if rel_path.parts[1] in ("pmtiles", "styles"):
                    rel_path = Path(*rel_path.parts[1:])
            
            # Deine Original-Logik: Fonts zusammenfassen
...
log_success "Endpunkt-Info aktualisiert."
